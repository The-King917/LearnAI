...
import { buildDiagnosticSystem, buildDiagnosticTools } from "@/lib/agents/diagnostic";
...
export async function POST(req: NextRequest) {
  ...
  if (!rateLimit(`agent-diag:${userId}`, RATE_LIMIT, RATE_WINDOW_MS)) {
  ...
  const { subjectId, messages: rawMessages, weeksUntilCompetition } = body as {
    subjectId?: string;
    messages?: unknown[];
  ...
  const tools = buildDiagnosticTools(subjectId);
  const system = buildDiagnosticSystem(subjectId, studentModel, weeksUntilCompetition);

  return streamResponse(runAgent(system, messages, tools, ctx));
}
